...
async function askAI(apiKey: string, model: string, input: string) {
  const response = await fetch("https://api.openai.com/v1/responses", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: \`Bearer \${apiKey}\` },
    body: JSON.stringify({ model, input, max_output_tokens: 12000 }),
  });
  if (!response.ok) throw new Error((await response.text()) || \`OpenAI API: \${response.status}\`);
  return response.json();
}

function parseAgentPlan(text: string) {
  const cleaned = text.replace(/^\`\`\`json\s*/i, "").replace(/\s*\`\`\`$/i, "").trim();
  const start = cleaned.indexOf("{"), end = cleaned.lastIndexOf("}");
  if (start < 0 || end < start) throw new Error("لم يُرجع الذكاء الاصطناعي خطة JSON صالحة.");
  return JSON.parse(cleaned.slice(start, end + 1));
}

function AIAgent({ repos, token, onDone }: { repos: Repo[]; token: string; onDone: () => void }) {
  ...
  const run = async () => {
    ...
    const repo = repos.find((r) => r.full_name === repoName);
    if (!repo) return;
    setRunning(true); setResult(""); setLog("يحلل الأمر...");
    localStorage.setItem(AI_KEY, apiKey.trim());
    localStorage.setItem(AI_MODEL_KEY, model.trim());
    try {
      let transcript = "";
      for (let round = 0; round < 6; round++) {
        const prompt = \`أنت وكيل هندسي داخل GitHub Viewer.
المستودع: \${repo.full_name}
الفرع: \${repo.default_branch}
أمر المستخدم: \${command}
وضع التنفيذ: \${execute ? "مسموح بالتعديل" : "قراءة وتحليل فقط"}

قواعد:
- لا تخترع محتوى الملفات. استخدم read_file عندما تحتاج ملفًا.
- المسارات النسبية فقط؛ ممنوع .. أو / في البداية.
- لا تحذف ملفات.
- في وضع القراءة أعد read_file فقط.
- في وضع التنفيذ، بعد القراءة يمكنك إعادة update_file بمحتوى الملف الكامل الجديد.
- لا تضع أسرارًا داخل الملفات.
- حد أقصى 3 ملفات تعديل.
- أعد JSON فقط:
{"message":"شرح مختصر","actions":[{"type":"read_file","path":"..."},{"type":"update_file","path":"...","content":"المحتوى الكامل","reason":"..."}]}
\${transcript}\`;

        const plan = parseAgentPlan(extractAIText(await askAI(apiKey.trim(), model.trim(), prompt)));
        const actions = Array.isArray(plan.actions) ? plan.actions : [];
        if (!actions.length) { setResult(plan.message || "انتهى التحليل."); break; }

        const updates: any[] = [];
        let didRead = false;
        for (const action of actions) {
          const path = String(action.path || "");
          if (!path || path.includes("..") || path.startsWith("/")) throw new Error("مسار ملف غير آمن.");
          if (action.type === "read_file") {
            const file = await githubFetch<FileResponse>(\`/repos/\${repo.full_name}/contents/\${path}?ref=\${encodeURIComponent(repo.default_branch)}\`, token);
            const decoded = file.content ? (() => {
              try {
                const binary = atob(file.content.replace(/\n/g, ""));
                return new TextDecoder().decode(Uint8Array.from(binary, (c) => c.charCodeAt(0)));
              } catch { return "[تعذر فك الملف كنص]"; }
            })() : "";
            transcript += \`\\n[FILE \${path}]\\n\${decoded.slice(0, 60000)}\\n[/FILE]\\n\`;
            didRead = true; setLog(\`قرأ: \${path}\`);
          } else if (action.type === "update_file" && execute) {
            const newContent = String(action.content ?? "");
            if (!newContent) throw new Error("محتوى التعديل فارغ.");
            if (updates.length >= 3) throw new Error("الحد الآمن هو 3 ملفات في العملية.");
            updates.push({ path, content: newContent });
          }
        }

        if (updates.length) {
          for (const u of updates) {
            const existing = await githubFetch<FileResponse>(\`/repos/\${repo.full_name}/contents/\${u.path}?ref=\${encodeURIComponent(repo.default_branch)}\`, token);
            await githubFetch<any>(\`/repos/\${repo.full_name}/contents/\${u.path}\`, token, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                message: \`AI: \${command.slice(0, 60)}\`,
                content: btoa(unescape(encodeURIComponent(u.content))),
                sha: existing.sha,
                branch: repo.default_branch
              })
            });
            setLog(\`تم تعديل: \${u.path}\`);
          }
          setResult(\`تم تنفيذ \${updates.length} تعديلًا في \${repo.full_name}.\`);
          onDone();
          break;
        }
        if (!didRead) { setResult(plan.message || "انتهى التنفيذ دون تعديلات."); break; }
        setLog("يراجع الملفات...");
      }
    } catch (e) { setResult(e instanceof Error ? e.message : "فشل وكيل الذكاء الاصطناعي."); }
    finally { setRunning(false); }
  };

  return (
    <section className="mt-6 rounded-3xl border border-fuchsia-400/20 bg-fuchsia-500/5 p-5 sm:p-6">
      <div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-fuchsia-500/20 text-xl">🧠</div><div><h2 className="text-xl font-bold">وكيل الذكاء الاصطناعي</h2><p className="mt-1 text-sm text-slate-400">أعطه أمرًا، فيقرأ ملفات المشروع ويحللها، ثم يعدلها فقط عند تفعيل التنفيذ.</p></div></div>
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="OpenAI API Key" className="rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none" dir="ltr" autoComplete="off" />
        <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="gpt-5.6-luna" className="rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none" dir="ltr" />
        <select value={repoName} onChange={(e) => setRepoName(e.target.value)} disabled={running} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 text-sm" dir="ltr">{repos.map((r) => <option key={r.id} value={r.full_name}>{r.full_name}</option>)}</select>
      </div>
      <textarea value={command} onChange={(e) => setCommand(e.target.value)} placeholder="مثال: افحص سبب مشكلة الأذان في هذا المشروع وحدد الملفات المتعلقة بها، ولا تعدل شيئًا." className="mt-3 min-h-28 w-full rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none" />
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={execute} onChange={(e) => setExecute(e.target.checked)} disabled={running} /> السماح بالتعديل وإنشاء commits</label>
        <button onClick={run} disabled={!apiKey.trim() || !command.trim() || !repoName || running} className="rounded-xl bg-gradient-to-r from-fuchsia-500 to-indigo-600 px-5 py-3 font-semibold text-white disabled:opacity-40">{running ? "الوكيل يعمل..." : "تنفيذ الأمر"}</button>
      </div>
      {log && <div className="mt-4 rounded-xl bg-black/20 p-3 text-xs text-slate-400">{log}</div>}
      {result && <div className="mt-3 rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-slate-300 whitespace-pre-wrap">{result}</div>}
      <p className="mt-3 text-xs text-amber-300/80">مهم: مفتاح OpenAI يُحفظ محليًا في هذا المتصفح ولا يُرسل إلى GitHub. للإنتاج يُفضّل نقل استدعاء AI إلى خادم آمن.</p>
    </section>
  );
}
